...
def test_plan_limits_row_shows_each_free_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # [fairness] exact label/tier wording and the floor-vs-round choice dropped.
    resets = datetime.fromisoformat(windows["daily-large"]["resets_at"].replace("Z", "+00:00"))
    assert resets.astimezone(UTC).time() == datetime.min.time()
    assert "daily-unlisted" not in windows


def test_plan_limits_row_survives_an_unreadable_ledger(_ledger: Path) -> None:
    # [fairness] break the ledger itself instead of patching an internal function.
    _ledger.write_text("{ this is not json", encoding="utf-8")
    row = plan_limits._openai_provider()
    # [fairness] "survives": no exception, a well-formed row. Showing an error or
    # treating the unreadable ledger as empty are both defensible.
    assert row["state"] in ("ok", "error")
    assert isinstance(row["windows"], list)
