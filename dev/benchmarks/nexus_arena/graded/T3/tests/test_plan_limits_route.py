...
def _clear_cache(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    # [fairness] private-state resets removed: every graded test runs in its own process.
    monkeypatch.setattr(
...
def test_claude_rate_limit_missing_retry_after_defaults_to_60(
    ...
    # [fairness] the exact 60 s default is not derivable; only "retries within ~a minute" is kept.
    clock.advance(61)
    res3 = _collect()
...
def test_claude_rate_limit_no_reading_gives_error_rate_limited(
    ...
    assert claude["windows"] == []
    # [fairness] undisclosed "reason" code and "retry_at" field dropped.
...
def test_claude_cooldown_logs_warning_once_and_info_on_recovery(
    ...
    def ours(level: int) -> list[Any]:
        return [r for r in caplog.records if r.name == plan_limits.logger.name and r.levelno == level]

    # [fairness] only this module's records (httpx logs every request at INFO);
    # the INFO-on-recovery log was never asked for.
    with caplog.at_level(logging.INFO):
        # 1. Initial success - no info logged (routine fetch)
        _collect()
        assert not ours(logging.WARNING)

        # 2. Trigger 429
        clock.advance(61)
        _collect()
        warnings = ours(logging.WARNING)
        ...
        monkeypatch.setattr(plan_limits, "_cache", None)
        _collect()
        assert len(ours(logging.WARNING)) == 1

        # 4. Recovery: still exactly one warning
        clock.advance(25)
        _collect()
        assert len(ours(logging.WARNING)) == 1
...
def test_claude_cache_persists_to_disk_and_reloads_after_restart(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if should_fail:
            # [fairness] a 429 (the stated problem), not a 500: this test is about surviving a restart.
            return httpx.Response(429, headers={"retry-after": "30"})
    ...
    # 2. Simulate restart: a fresh module (whatever its state is called), next upstream call fails
    # [fairness] reload instead of resetting the author's private variable names.
    import importlib

    importlib.reload(plan_limits)
    monkeypatch.setattr(plan_limits, "CLAUDE_CACHE_PATH", cache_file)
    monkeypatch.setattr(plan_limits, "_read_claude_token", lambda: "tok")
    monkeypatch.setattr(plan_limits, "_cache", None)
    _stub_agy(monkeypatch, None)
