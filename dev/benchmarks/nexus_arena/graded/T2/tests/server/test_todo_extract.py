...
def _cs(todos):
    """[fairness] compare only the disclosed keys; the extra ``activeForm`` value is the author's choice."""
    out = []
    for t in todos or []:
        get = t.get if isinstance(t, dict) else (lambda k, _t=t: getattr(_t, k, None))
        out.append({"content": get("content"), "status": get("status")})
    return out
...
"""
    ...
    # [fairness] "- [ ] Task 5" under "### Done": whether the heading overrides the box is the
    # author's choice, so that one item is left out of the comparison.
    got = [t for t in result.todos if not t["content"].startswith("Task 5")]
    assert len(got) == 5
    assert got == [x for x in [
        {
    ...
    ] if not x["content"].startswith("Task 5")]
...
def test_build_session_response_rebuilds_todos_on_cache_miss() -> None:
    ...
    assert _cs(resp.todos) == [{"content": "Task A", "status": "pending"}]
