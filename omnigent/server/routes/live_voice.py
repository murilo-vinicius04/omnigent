...
def create_live_voice_router(*, auth_provider: AuthProvider | None = None) -> APIRouter:
    ...
    async def live_offer(request: Request, body: LiveOfferRequest) -> LiveOfferResponse:
        ...
        try:
            session_id, answer = await open_session(
                sdp_offer=body.sdp,
                instructions=body.instructions or DEFAULT_INSTRUCTIONS,
                model=body.model,
                voice=body.voice,
            )
        ...
        return LiveOfferResponse(session_id=session_id, sdp=answer)
