...
# Claude Code (2.1.280) withholds its task-list tools (TaskCreate, TaskUpdate,
# TaskList, TaskGet) from Opus 5 and 5.5 by default; Haiku gets them. The web UI
# mirrors that list in its Todo panel, which is how a reader follows long work,
# so ask for the tools on every model.
_CLAUDE_CODE_ENABLE_TODO_TOOLS_ENV = "CLAUDE_CODE_ENABLE_TODO_TOOLS"
...
def build_native_claude_terminal_env(
    ...
    :data:`_CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY_ENV`), and turns on the
    task-list tools the Todo panel mirrors (see
    :data:`_CLAUDE_CODE_ENABLE_TODO_TOOLS_ENV`).
    ...
    terminal_env = {
        ...
        _CLAUDE_CODE_ENABLE_TODO_TOOLS_ENV: "1",
    }
    if claude_config is not None:
        ...
        terminal_env[_CLAUDE_CODE_ENABLE_TODO_TOOLS_ENV] = "1"
